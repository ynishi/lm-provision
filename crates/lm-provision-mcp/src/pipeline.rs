...
//! `lm-provision` library's own CLI pipeline functions
//! ([`lm_provision::cli::validate_pipeline`] etc.), since `profile_path`
//! is a path visible to the MCP server host (10 §Inputs: "profile
//! authoring/upload is out of scope") and none of these three
//! subcommands touches a bridge or a pod (07-cli.md §Invocation: "none
...
//! failure these three pipeline stages can raise (profile load, Lua
//! eval, validate rejection) is precondition-class end to end — the
...
use lm_provision::cli::{hash_pipeline, plan_pipeline, validate_pipeline};
...
pub fn lm_validate(profile_path: &Path) -> Result<serde_json::Value, String> {
    let name = validate_pipeline(profile_path).map_err(|err| err.to_string())?;
...
pub fn lm_hash(profile_path: &Path) -> Result<serde_json::Value, String> {
    let hash = hash_pipeline(profile_path).map_err(|err| err.to_string())?;
...
pub fn lm_plan(profile_path: &Path) -> Result<serde_json::Value, String> {
    plan_pipeline(profile_path).map_err(|err| err.to_string())
}
...
mod tests {
    ...
    fn lm_validate_happy_returns_ok_true_and_the_profile_name() {
        let value = lm_validate(&fixture("valid.lua")).expect("valid.lua should validate");
    ...
    fn lm_validate_missing_file_is_a_precondition_class_error() {
        let err = lm_validate(Path::new("/nonexistent/lm-provision-profile.lua"))
            .expect_err("a missing profile file must not validate");
        assert!(
            err.contains("failed to read profile file") || err.contains("No such file"),
    ...
    fn lm_hash_returns_a_64_char_lowercase_hex_digest() {
        let value = lm_hash(&fixture("valid.lua")).expect("valid.lua should hash");
    ...
    fn lm_hash_is_deterministic_across_repeated_calls() {
        let first = lm_hash(&fixture("valid.lua")).expect("first hash");
        let second = lm_hash(&fixture("valid.lua")).expect("second hash");
        assert_eq!(first["hash"], second["hash"]);
    }

    #[test]
    fn lm_plan_returns_the_plan_artifact_as_a_json_object_with_steps() {
        let value = lm_plan(&fixture("valid.lua")).expect("valid.lua should plan");
    ...
    fn lm_plan_missing_file_is_a_precondition_class_error() {
        let err = lm_plan(Path::new("/nonexistent/lm-provision-profile.lua"))
