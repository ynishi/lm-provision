...
| `lm_cost` | `provider?` string; `model?` string; `endpoint?` string; `endpoints_file?` string; `acquisitions?` string; `forwards?` string; `usage?` object; `usage_format?` string; `at?` string; `period?` string; `prices?` string | 09 §Cost (read-only) |
